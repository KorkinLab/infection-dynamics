...
//        }
        ...
        //DO THIS STUFF IF YOU ARE A ZOMBIE OR THE TOTAL INFECTED IS > (1/3) TOTAL POPULATION
//        if(state.getRunCode() == 2 && state.getTime() > 0.0 && this.location.getIntegerAttribute("INFECTED") > 0)//if zombie, wander or follow humans
//        {
//            Coordinate humanCoord = null;
//            Bag humans = state.agents.getObjectsWithinDistance(this.location, foot*25);
//            MasonGeometry nearAgent = (MasonGeometry)humans.pop();
//
//            while(nearAgent != null && nearAgent.getIntegerAttribute("INFECTED") > 0)
//            {
//                humans.shuffle(state.random);
//                nearAgent = (MasonGeometry)humans.pop();
//            }
//
//            int isSafe = 0;
//            if(nearAgent != null)
//                isSafe = nearAgent.getIntegerAttribute("SAFE");
//
//            if(isSafe == 1 || nearAgent == null)//can't touch them in safe rooms
//            {
//                System.out.println("Not going to safe node");
//                return this.getRandomNode(state);
//            }
//
//            humanCoord = nearAgent.getGeometry().getCoordinate();
//            Node humanNode = null;
//            if((humanNode = state.network.findNode(humanCoord)) != null)
//            {
//                System.out.println("Going to nearest human");
//                return humanNode;//BRAINS
//            }
//            else
//                return this.getRandomNode(state);//still hungry
//        }
//        else if(state.getRunCode()==2 && state.getTime() > 0.0
//               && this.location.getIntegerAttribute("INFECTED") == 0
//               && ((double)state.getNumInfected()/(double)state.getNumAgents()) >= (1.0/3.0))
//        {
//            return this.safeNode;
//        }
