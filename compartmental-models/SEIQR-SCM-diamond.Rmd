---
title: "Stratified (passengers, crew) SEIQR SCM for Diamond Princess (COVID-19)"
---
...
all_param_values <- c(vspf=0.50, qfrac=0.25, qrate=1/2, 
                      rho=0.99, psi=0.1, mixpc=0.75,
...
```{r}
# Load observed data
data <- read.csv("diamond-data.csv")
data
```
...
rproc <- Csnippet("
  ...
  Cp += deltap[1]; // Cumulative number of cases by adding up E -> I delta

  // Force of infection for crew members
  lambdac = reff * gamma * (Ic/Nc+mixpc*Ip/Np);
  ...
  Cc += deltac[1]; // Cumulative number of cases by adding up E -> I delta
")
...
# POMP model
simulate(t0=1, times=1:num_days,
         ...
         cdir=code_folder,
         cfile=file_name,       
         # Cp and Cc should NOT be accumulator variables:
         # https://search.r-project.org/CRAN/refmans/pomp/html/accumvars.html
         # becasuse otherwise they are reset every time step.
         #accumvars=c("Cp", "Cc"),
...
p <- plot_sim(sim_data, "Cumulative COVID-19 Cases on Diamond Princess")
ggsave(file.path(out_folder, "trajectories.pdf"), plot = p, width = 8, height = 6)
```

```{r}
for (R0 in c(2.1, 2.5, 3.0, 3.5)) {
  all_param_values["vspt"] <- 0
  all_param_values["vspf"] <- 1 #fraction by which R0 is reduced as a result of cleaning, etc. 0.5; for SEIR_DCM  = 1.0, that i, no VSP is carried out
  all_param_values["R0"] <- R0
  model_sim  %>%
    simulate(nsim=num_sims, params=all_param_values) -> sim_data
  plot_sim(sim_data, paste0("Cumulative COVID-19 Cases for R0 = ", R0))
}
```
